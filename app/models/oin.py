...
_OIN_PATTERN = re.compile(r"^\d{8}(?:[A-Za-z0-9]{8}0{4}|[A-Za-z0-9]{9}0{3})$")
...
class Oin:
    ...
    def __init__(self, value: Any) -> None:
        ...
        if not _OIN_PATTERN.match(str_value):
            raise ValueError(
    ...
    def mainnumber(self) -> str:
        ...
        if self.number.endswith(self._LONG_SUFFIX):
            return self.number[: self.TOTAL_LENGTH - self.PREFIX_LENGTH - len(self._LONG_SUFFIX)]
        return self.number[: self.TOTAL_LENGTH - self.PREFIX_LENGTH - len(self._SHORT_SUFFIX)]
